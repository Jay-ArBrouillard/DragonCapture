var armDir;
var invulnerableTimer = 0; //Don't want to get damaged too quickly in succession
function FuelCarrier(x, y, z, degrees, texSrc) {
    GameObj2.call(this, x, y, z, degrees, 2, degrees);
    const _ = this;
    _.scale = .1;
    _.mesh = FuelCarrier.mesh;
...
FuelCarrier.prototype = Object.create(GameObj2.prototype);
// make degrees [0-360)
FuelCarrier.prototype.fixDegrees = function () {
    const _ = this;
    _.degrees %= 360;
    _.xdir = Math.cos(_.degrees * Math.PI / 180.0);
    _.zdir = Math.sin(_.degrees * Math.PI / 180.0);
};
FuelCarrier.prototype.turn = function (degreesRotation) {
    const _ = this;
...
FuelCarrier.prototype.update = function () {
    const _ = this;
    let move = _ == G.objects.hero ? K.speed.hero : K.speed.villain;
    ...
    _.fixDegrees();
    if (this == G.objects.hero) {

        if (G.keys[87]) {
            _.move(+move); // forward
        }
        if (G.keys[83]) {
            _.move(-move); // backward
        }
        if (G.keys[65]) {
            _.turn(-K.speed.turn); // left
        }
        if (G.keys[68]) {
            _.turn(+K.speed.turn); // right
        }
        ...
        for (var i = 0; i < G.objects.clouds.length; i++) {
            if (G.objects.target.isCollide(G.objects.clouds[i]) && G.objects.target.y > 0 ) {
                if (closestBoost != null) {
                    ...
                    target = new FuelCell(Math.random()*300, 0, Math.random()*300, 0);
                }
            }
    ...
    //Collide with rocket
    if (G.objects.hero.isCollide(G.objects.rocket.rocket)) {
        const centerX = (G.objects.hero.x + G.objects.rocket.rocket.x) / 2;
        const centerZ = (G.objects.hero.z + G.objects.rocket.rocket.z) / 2;
    ...
    //collide with rock
    if (invulnerableTimer != 0 && invulnerableTimer < 120) {
        invulnerableTimer++;
    ...
    if (_.isCollide(G.objects.rock.rock)) {
        const centerX = (G.objects.hero.x + G.objects.rock.rock.x) / 2;
        const centerZ = (G.objects.hero.z + G.objects.rock.rock.z) / 2;
        let v = [G.objects.hero.x - centerX, G.objects.hero.z - centerZ];
        normalize(v);
        v[0] *= -0.5;
        v[1] *= -0.5;
...
FuelCarrier.prototype.show = function (degree) {
    armDir = G.heroSpin;
    const _ = this;
    const d = _.degrees; // save degrees
    if (degree != undefined) _.degrees = degree; // rotate mesh
    if (invulnerableTimer > 0 || _.boosterCounter > 0) {
        _.glow[3] = 1;
