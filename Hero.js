var heroInvulnerableTimer = 0; //Don't want to get damaged too quickly in succession
function Hero(x, y, z, degrees, texSrc) {
...
Hero.prototype = Object.create(GameObj2.prototype);
// make degrees [0-360)
Hero.prototype.fixDegrees = function () {
    const _ = this;
    _.degrees %= 360;
    _.xdir = Math.cos(_.degrees * Math.PI / 180.0);
    _.zdir = Math.sin(_.degrees * Math.PI / 180.0);
};
Hero.prototype.turn = function (degreesRotation) {
    const _ = this;
...
Hero.prototype.update = function () {
    ...
    //collide with rock
    if (heroInvulnerableTimer != 0 && heroInvulnerableTimer < 120) {
        heroInvulnerableTimer++;
        _.glow = [0.0,0.0,1.0,1.0]; // blue Color, on
    } else {
        heroInvulnerableTimer = 0;
        _.glow = [1.0,0.0,0.0,0.0]; // red Color, off
    ...
    if (_.fuelCell) {
        if (_.fuelCell == G.objects.rock.rock) {
            _.fuelCell.x = _.x - Math.sin(radians(G.heroSpin)) * (_.bounding_cir_rad + 2);
            _.fuelCell.z = _.z - Math.cos(radians(G.heroSpin)) * (_.bounding_cir_rad + 2);
            _.fuelCell.y = 1;
        } else {
            _.fuelCell.x = _.x - Math.sin(radians(G.heroSpin)) * (_.bounding_cir_rad + 1);
            _.fuelCell.z = _.z - Math.cos(radians(G.heroSpin)) * (_.bounding_cir_rad + 1);
            _.fuelCell.y = 0;
        }
    }
...
Hero.prototype.show = function (degrees) {
    const _ = this;
    const d = _.degrees; // save degrees
    if (degrees != undefined) _.degrees = degrees; // rotate mesh
    if (heroInvulnerableTimer > 0 && _.boosterCounter > 0) {
        _.glow = [1.0, 0.0, 1.0, 1.0];
    } else if (heroInvulnerableTimer > 0) {
