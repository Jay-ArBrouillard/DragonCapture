...
window.onkeydown = function (event) {
    ...
    if (event.keyCode == 80) {
        ...
        G.score.villain - 3 >= G.score.hero || G.objects.hero.health <= 0 || G.objects.villain.health <= 0) { //Game over restart game
        K.speed.villain = 0.6;
        G.score.hero = 0;
        G.score.villain = 0;
        K.display321 = true;
...
function drawOverlay() {
    ...
    if (G.score.hero - 3 >= G.score.villain || G.objects.villain.health <= 0) {  //Hero win
        g.globalAlpha = 0.666;
        g.fillStyle = "#000";
        ...
        K.speed.villain = 0.0;
    } else if (G.score.villain - 3 >= G.score.hero || G.objects.hero.health <= 0) { //Villain win
        g.globalAlpha = 0.666;
        g.fillStyle = "#000";
        g.fillRect(0, 0, width, height);
        g.globalAlpha = 1;
